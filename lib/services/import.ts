...
function validateData(json: Record<string, unknown>): { valid: boolean; error?: string } {
  if (!json.metadata || typeof json.metadata !== "object") {
    return { valid: false, error: "El archivo no tiene el formato correcto (falta metadata)." }
  }
  if (!json.notes || typeof json.notes !== "object") {
    return { valid: false, error: "El archivo no tiene el formato correcto (falta notes)." }
  }
  return { valid: true }
}
