...
import type { Task, List, Project, Area, Workspace, Priority } from "@/lib/types/task"
...
const defaultState = {
  workspaces: [{ id: defaultWorkspaceId, name: "Mi Workspace", areaIds: [defaultAreaId] }],
  areas: [{ id: defaultAreaId, workspaceId: defaultWorkspaceId, name: "General", projectIds: [defaultProjectId] }],
  projects: [{ id: defaultProjectId, areaId: defaultAreaId, name: "Proyecto inicial", listIds: [] }],
  lists: defaultLists,
  ...
  persist(
    (set, get) => ({
      ...
      getTasksByArea: (areaId) => {
        const state = get()
        const projectIds = state.projects.filter((p) => p.areaId === areaId).map((p) => p.id)
        return Object.values(state.tasks).filter((t) => t.projectId && projectIds.includes(t.projectId))
      },

      getTasksByPriority: (priority) => {
        const state = get()
        return Object.values(state.tasks).filter((t) => t.priority === priority)
      },
