...
import type { Task, Project, List } from "@/lib/types/task"
...
export function TaskList({ tasks, projects, lists }: TaskListProps) {
  ...
  if (areaId) {
    const projectIds = projects.filter((p) => p.areaId === areaId).map((p) => p.id)
    filteredTasks = filteredTasks.filter((t) => t.projectId && projectIds.includes(t.projectId))
  }
